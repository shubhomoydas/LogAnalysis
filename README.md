...
To run (make a folder 'output' in order for output files to be generated):
    ...
    spark-submit --master local[4] target/LogAnalysis-0.1.0-jar-with-dependencies.jar <operation> [ip address]
...
Quick notes on hard-coded assumptions:

1. Max time between two URLs within a session for a user is assumed
    to be 30 mins.

2. The next minute prediction averages load from past 5 minutes.
    The predictions by operation 6 match quite well with true values. See: https://github.com/shubhomoydas/LogAnalysis/blob/master/docs/predicted_server_load.pdf
...
A sample command for computing average session time, nav. length, unique URLs with operation '3' is:
    ...
    spark-submit --master local[4] target/LogAnalysis-0.1.0-jar-with-dependencies.jar 3 14.141.255.74
...
A sample command for computing average session duration with operation '7' is:
    ...
    spark-submit --master local[4] target/LogAnalysis-0.1.0-jar-with-dependencies.jar 7 220.226.206.7
...
A sample command for computing average # session unique urls with operation '8' is:
    ...
    spark-submit --master local[4] target/LogAnalysis-0.1.0-jar-with-dependencies.jar 8 117.195.91.36
